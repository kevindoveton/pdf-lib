...
import { distance, isColinear, isEqual, norm, orthogonal, times, unitVector, vector, rotate, } from './maths.js';
...
export const intersection = (A, B) => intersections(A, B)[0];
...
export const intersectionLine = (A, B) => {
...
export const intersectionCircleLine = (A, B) => {
    const rA = A.ray();
    const O = A.center();
...
export const intersectionCircle = (A, B) => {
    const oA = A.center();
    const oB = B.center();
    const rA = A.ray();
    const rB = B.ray();
...
export const getIntersections = (elements) => {
    const checked = [];
    const inters = [];
